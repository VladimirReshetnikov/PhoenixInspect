...
`proposals/architecture/counterfactual-method-evaluation-contract-proposal.md`. W4.1's fixture gate is implemented and
locally validated at pushed checkpoint `82363585b`; W4.2–W4.9 and counterfactual product execution have not landed.
...
same/fresh-session replay gates remain required future headless evidence, not current product results. W4.1 currently
contributes four fast fixture facts: exact caller/helper IL and metadata closure, CoreCLR agreement, and truthful W3
rejection at the second field load before the still-unadmitted direct call. Locked restore, zero-warning Release build,
focused 4/4, complete fast 71/71, and ordinary-dump 5/5 all passed locally and headlessly under
`Scope!=Cybersecurity`; the realized slice is 478 added or materially revised LOC.
...
| `proposals/product/post-mortem-debugging-feature-proposal.md` | Product | Proposal | Draft · Active | Active read-only dump evaluator, including W4.1's landed fixture gate and the admitted-but-unimplemented W4.2–W4.9 execution slices, plus explicitly gated research phases. |
...
| `proposals/architecture/counterfactual-method-evaluation-contract-proposal.md` | Architecture | Contract | Current · Active | Normative W4 branchless `GetMarkerSummary`/`CombineMarkers` method-evaluation contract; W4.1 gate evidence is local/pushed and W4.2–W4.9 remain pending. |
