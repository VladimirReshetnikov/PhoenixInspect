...
session (`phoenixinspect --attach <pid>`; the Processes pane in the shell). Everything above the adapter is
...
| Processes | `ProcessDiscoveryService` | The running .NET processes a live-attach session could inspect, as a dockable pane rather than a modal dialog: it stays open, filters by name or id, refreshes on demand, and attaches from the selection (button or double-click). Each row states *how* the process was recognised as managed — its runtime published a diagnostics endpoint, or a runtime module is loaded in it — and a process of the wrong bitness is listed with that reason instead of being silently dropped. Detection reads no target memory and suspends nothing; only the attach itself does. |
...
| Watch 1 | `ExpressionEvaluationService.EvaluateWatch`, `ExpressionCompletionService` | Editable watch expressions, like Visual Studio's Watch window: every row's expression edits in place, the trailing row adds a new watch, and rows re-evaluate when the adopted frame context, root, or root identifier changes. Which entry point answers is the API's single lexical rule — an expression referencing the adopted root's identifier evaluates root-relatively, everything else through the static-field path — and each row keeps its complete report. Compound values — arrays and tuples — expand into `[i]` and `ItemN`/named child rows realized by the report itself, recursively for nested compounds and bounded with an honest tail row. Typing opens a completion drop-down: C# keywords, the modeled type receivers and their dispatched members, the adopted root's declared fields from its runtime type, and namespaces, types, and static fields read from dump-module metadata — every candidate either evaluates or produces its own explained stop. Expressions survive dump close and reopen. |
| Immediate | `ExpressionEvaluationService.EvaluateWatch`, `CompletionSessionState` | An Immediate window, like Visual Studio's: type an expression at the prompt and press Enter; the expression and its answer append to a C#-syntax-colored transcript. Values carry their kind as a trailing comment; a non-exact answer renders as comment lines holding the status, stage, and stable diagnostics, so a stop reads as an explanation. The same completion drop-down as the Watch window opens as you type — one shared, presentation-agnostic completion state serves every expression editor. ↑/↓ recall history; `cls` clears. |
