...
| `System.Collections.Immutable`: arrays, lists, sets, queues, stacks | Factories (`ImmutableArray.Create(1, 2)`, `ImmutableList.CreateRange(xs)`, `ImmutableSortedSet<int>.Empty`), the `ToImmutableArray`/`ToImmutableList`/`ToImmutableHashSet`/`ToImmutableSortedSet` extensions, and the persistent instance surface — `Add`/`SetItem`/`Remove` return a new collection, sets keep the set algebra (`Union`, `SymmetricExcept`, `SetEquals`…), queues and stacks answer `Enqueue`/`Dequeue`/`Push`/`Pop`/`Peek` — with each kind's exact properties (`Length` vs `Count` vs `IsEmpty`), indexer fidelity, LINQ over every kind, `typeof`/`GetType`/`is` identity (`ImmutableList.Create(1) is IImmutableList<int>`), and storage as Immediate variables. Hash-set enumeration is modeled as first-insertion order (the BCL leaves it unspecified); a sorted set of strings is a typed stop, as default string ordering is culture-sensitive |
