...
| `System.Collections.Immutable`, the whole family | Factories (`ImmutableArray.Create(1, 2)`, `ImmutableList.CreateRange(xs)`, `ImmutableSortedSet<int>.Empty`, `ImmutableDictionary.Create<int, string>()`), the `ToImmutableArray`/`ToImmutableList`/`ToImmutableHashSet`/`ToImmutableSortedSet`/`ToImmutableDictionary`/`ToImmutableSortedDictionary` extensions (including the key/value-selector overloads), and the persistent instance surface — `Add`/`SetItem`/`Remove` return a new collection, sets keep the set algebra (`Union`, `SymmetricExcept`, `SetEquals`…), queues and stacks answer `Enqueue`/`Dequeue`/`Push`/`Pop`/`Peek`, dictionaries answer key lookup (`d[key]` with the exact `KeyNotFoundException`), `ContainsKey`/`ContainsValue`, and the `Keys`/`Values` projections over first-class `KeyValuePair` values (`KeyValuePair.Create(1, "a").Key`, `d.Sum(pair => pair.Value)`) — with each kind's exact properties (`Length` vs `Count` vs `IsEmpty`), indexer fidelity, LINQ over every kind, `typeof`/`GetType`/`is` identity (`ImmutableList.Create(1) is IImmutableList<int>`), and storage as Immediate variables. Hash-set enumeration is modeled as first-insertion order (the BCL leaves it unspecified); sorted kinds order with the deterministic comparison, so culture-sensitive string ordering stays a typed stop |
