...
\begin{itemize}
  ...
  \item a deterministic constant evaluator covering the C\# numeric tower, strings, chars, arrays, virtual sequences, tuples, anonymous objects, enums, \codeinline{typeof}, temporal values, \typeName{Guid}, \typeName{Version}, query expressions, lambdas, patterns, interpolated strings, and selected pure BCL operations;
...
\repoPath{PhoenixInspect.Product.DumpQuery} & Roslyn parsing and project-owned admission, rooted queries, member chains, static-field binding/evaluation, the deterministic constant evaluator, metadata-authority catalogs, and edit-generation contracts. \\
...
For enum constants and literal fields, the constant evaluator reads the declaring module's metadata Constant table. It does not use the analysis runtime's reflection view of a same-named enum. This is a subtle but essential guarantee: an old dump can be inspected on a machine with newer framework assemblies without silently changing the enum member map.
...
Arrays are represented as immutable virtual sequences when their shape and element reads can be established. The constant evaluator can consume those sequences through deterministic operators without copying target objects into the analysis runtime's ordinary collection semantics. Bounds are retained for dimensions, lengths, element reads, and result materialization.
...
\chapter{Parsing, binding, rooted queries, and constant evaluation}
\label{ch:query}
...
\begin{itemize}
  ...
  \item a large deterministic constant-expression evaluator;
...
A bare dump field or member chain deliberately remains outside the constant pre-stage so its existing evidence report and replay digest stay byte-identical. A composed expression such as \codeinline{root.QueueDepth * 2 + 1} enters the constant evaluator, which calls the frozen root pipeline for the operand and consumes its exact value.
...
\begin{sourcebox}
Core reading order: \repoPath{DumpQueryParser.cs}; \repoPath{DumpQueryEngine.cs} and \repoPath{DumpQueryPlan.cs}; member-chain parser/plan/engine files; \repoPath{StaticFieldExpressionEvaluator.cs}; then the partial \repoPath{ConstantExpressionEvaluator.*.cs} families. Tests are organized similarly: parser/plan tests, \repoPath{ConstantExpression*Tests.cs}, and real-dump member/static-field suites.
\end{sourcebox}
...
\begin{description}[style=nextline]
  \item[Static field] Fully qualified or contextual static expressions, with the constant pre-stage and optional root promotion.
...
\begin{enumerate}
  ...
  \item The constant pre-stage asks whether the entire expression is a foldable composition. A bare stored field returns \status{NotConstant} and falls through unchanged.
...
\begin{enumerate}
  ...
  \item The constant pre-stage sees a bare chain and returns \status{NotConstant}, preserving the member-chain report.
...
This route reveals architecture before scale. Starting with the constant evaluator's many partials or the desktop XAML can make the repository look like an undifferentiated debugger application.
...
\codeinline{NotConstant} & Constant pre-stage fall-through preservation. \\
...
Where is folded C\# semantics implemented? & \repoPath{ConstantExpressionEvaluator.cs} and its partials by semantic family. \\
