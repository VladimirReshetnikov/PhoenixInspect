...
| Edit-and-Continue / Hot Reload dumps | The [Edit-and-Continue Support Plan](edit-and-continue-support-plan.md) is delivered to its completion definition: edited modules are detected physically and refused with typed stops end to end over really edited processes, delta artifacts acquire and chain on the caller-owned seam with the counter auditing them, and the effective projection models generation-aware table state over both measured edit vocabularies. The remaining arms — runtime edit-structure characterization, per-consumer composition over the projection, and the delta-PDB artifact seam — are frozen typed non-admissions recorded per stride in that plan; section 2 preserves the measured pre-E1 baseline. |
