# Original Integration Test Plan

> **Lifecycle:** Historical · **Roadmap relation:** Reference
>
> **Restored original plan.** This is the original pre-redesign integration-test plan,
> restored from repository history (tag `before-redesign`) after the 2026-07-13 scope reset, with project naming
> updated to `PhoenixInspect`. The slice it describes — the smallest real dump-to-`ret`-step integration test — has
> since been implemented and superseded. The document may be outdated and needs revision to match the architecture that has
> crystallized since then; where it disagrees with a current document or with executable evidence, the current
> source is authoritative. Current counterpart: [`../integration-test-plan.md`](../integration-test-plan.md).
...
> **Implementation status (2026-07): complete for the stated narrow slice.** The target and test are implemented in `tests/PhoenixInspect.TestTarget` and `tests/PhoenixInspect.IntegrationTests`; supporting code lives in `PhoenixInspect.Core.Execution`, `PhoenixInspect.Metadata.SRM`, and `PhoenixInspect.Host.Dump.ClrMD`. The test validates module discovery from a generated dump, then reads IL from the still-accessible on-disk module. It does not yet validate dump-backed heap reads, missing artifacts, frame seeding, or overlay memory.
...
* `tests/PhoenixInspect.TestTarget/PhoenixInspect.TestTarget.csproj`
...
Before you even generate a dump, your integration test should open the built `PhoenixInspect.TestTarget.dll` with the metadata reader and verify the method body bytes are exactly `0x2A`. If that fails, the test should stop immediately with a clear message (“expected ret-only; got …”).
...
In your integration test project (e.g., `tests/PhoenixInspect.IntegrationTests`):

* Start `PhoenixInspect.TestTarget` as a child process.
* Capture stdout.
...
* locate the module whose name/path ends with `PhoenixInspect.TestTarget.dll`
...
* `PhoenixInspect.Core.Execution` (the stepper / micro-step engine)
* A trivial `IValueDomain<TValue>` implementation:

  * easiest: use `PhoenixInspect.Domain.Concrete`
...
* mapping to a synthetic document `il://PhoenixInspect.TestTarget/Program.RetOnly`

Then drive:

* `PhoenixInspect.Debugger.Engine.StepInto` (or StepOver) once
...
* [ ] Create `PhoenixInspect.TestTarget` executable project
...
* [ ] `CreateRuntime`
* [ ] Find `PhoenixInspect.TestTarget.dll` module
...
4. **Find module** `PhoenixInspect.TestTarget.dll` in `runtime.Modules`
...
## 2.2 `PhoenixInspect.Host.Dump.ClrMD` (dump loading + module discovery)
...
public sealed record ClrmdModuleInfo(
    string Name,       // e.g. "PhoenixInspect.TestTarget.dll"
...
## 2.3 `PhoenixInspect.Metadata.SRM` (metadata + IL body extraction)
...
### A) `SrmMetadataModule` (implements `PhoenixInspect.Metadata.Abstractions.IMetadataModule`)
...
## 2.4 A minimal `PhoenixInspect.Core.Abstractions.IResolutionServices`
...
```csharp
public sealed class MetadataResolutionServices : PhoenixInspect.Core.Abstractions.IResolutionServices
{
    private readonly PhoenixInspect.Metadata.Abstractions.IMetadataModule _module;

    public MetadataResolutionServices(IMetadataModule module) => _module = module;

    public bool TryGetMethodBody(MethodHandle method, out PhoenixInspect.IL.MethodBody body)
...
## 2.5 `PhoenixInspect.Core.Execution` (the minimal IL execution engine)
...
1. Build path to `PhoenixInspect.TestTarget.exe`
2. Run target, wait READY
3. Write full dump
4. Load dump via `ClrmdDumpSession`
5. Find module path (`PhoenixInspect.TestTarget.dll`)
...
* [ ] `ClrmdDumpSession.TryFindModuleByFileName("PhoenixInspect.TestTarget.dll", out module)`
