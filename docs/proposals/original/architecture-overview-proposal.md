...
> updated to `PhoenixInspect`. The scope it describes — a multi-application execution platform spanning dump-time debugging, live speculative debugging, static analysis, and sandboxed (no-JIT) IL interpretation — is a tentative future direction, not an active
...
> source is authoritative. Current counterpart: [`../architecture/architecture-overview-proposal.md`](../architecture/architecture-overview-proposal.md).
