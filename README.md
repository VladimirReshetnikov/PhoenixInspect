...
- **W6 delivery status:** the detailed W6 and
  ...
  language/report behavior remains frozen, and the handwritten production reader is deleted. W6.3 now binds the
  declared reference target and certifies terminal direct fields or compiler-emitted data properties without reading
  the reference or terminal value. W6's umbrella scale is `~10K LOC` split into `~1K LOC` evidence checkpoints.
