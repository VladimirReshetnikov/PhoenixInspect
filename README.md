...
- **W6 delivery status:** the detailed W6 and
  ...
  TestTarget identity refresh is explicit. W6.2 now pins `Microsoft.CodeAnalysis.CSharp/5.3.0`, parses each bounded C#
  expression once, admits only ordered project-owned W2/W5/opt-in W6 tree shapes, preserves default W5
  language/report behavior, and deletes the handwritten production parser after differential compatibility. W6's
  umbrella scale is `~10K LOC` split into `~1K LOC` evidence checkpoints, with the front-end design and implementation
  each at `~1K LOC` scale.
