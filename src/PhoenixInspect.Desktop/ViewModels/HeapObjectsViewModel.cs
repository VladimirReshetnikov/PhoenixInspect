...
public sealed class HeapObjectsViewModel : ObservableObject
{
    ...
    /// <summary>Creates the heap-object pane.</summary>
    ...
    public HeapObjectsViewModel(IShellServices shell)
    {
        ...
        GroupedSearchFacts = new PropertyGroupList(SearchFacts);
    }

    /// <summary>Gets the last search's traversal facts grouped by section heading.</summary>
    public PropertyGroupList GroupedSearchFacts { get; }
    ...
    /// <summary>Clears every previous match so the pane matches a newly opened or closed dump.</summary>
    public void Reset()
    {
        Matches.Clear();
        SearchFacts.Clear();
