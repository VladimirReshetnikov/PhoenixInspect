...
public static partial class ExpressionEvaluator
{
    ...
    private static Type? RuntimeTypeOfReceiver(TypeReceiver receiver) => receiver.Category switch
    {
        ...
        TypeReceiverCategory.BclValue => receiver.Value switch
        {
            ...
            BclValueKind.DBNull => typeof(DBNull),
            _ => null,
        },
        TypeReceiverCategory.CharUnicodeInfo => typeof(System.Globalization.CharUnicodeInfo),
        TypeReceiverCategory.SystemConvert => typeof(Convert),
    ...
    private const string InvocationDepthCode = "EVAL_INVOCATION_DEPTH_EXCEEDED";
    ...
    /// proceeds, so every other non-member invocation shape keeps its existing not-folded path.
    /// </summary>
