...
{
    private const string PatternUnsupportedCode = "CONSTANT_PATTERN_UNSUPPORTED";
    private const string UncheckedUnsupportedCode = "CONSTANT_UNCHECKED_UNSUPPORTED";
                    ...
                    if (interpolation.AlignmentClause is { } alignmentClause)
                    {
                        ...
                        if (!TryImplicitInt32(alignment.Operand, out var width))
                        {
                            return FoldOutcome.Error(
                                OperandTypeCode,
                                "An interpolation alignment must be an Int32 constant.");
                        }
    ...
    private static FoldOutcome RenderInterpolatedValue(Operand operand, string? format)
    {
        switch (operand.Kind)
        {
            ...
            default:
                return FoldOutcome.Error(
                    OperandTypeCode,
                    "An interpolated value must be a scalar constant: a string, char, Boolean, number, null, or "
        ...
        switch (pattern)
        {
            ...
            case RelationalPatternSyntax relational:
                ...
                if (!value.IsNumeric || !bound.Operand.IsNumeric)
                {
                    return FoldOutcome.Error(
                        OperandTypeCode,
                        "A relational pattern requires numeric constant operands.");
                }
