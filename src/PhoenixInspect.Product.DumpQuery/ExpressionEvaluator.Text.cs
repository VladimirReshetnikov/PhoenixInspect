...
public static partial class ExpressionEvaluator
{
    ...
    private static FoldOutcome ByteArrayRequired(string member) => FoldOutcome.Error(
        OperandTypeCode,
        $"'{member}' requires an exact byte[] argument; write 'new byte[] {{ … }}' or compose a value whose "
        ...
        FoldContext context)
    {
        if (!IsRegexCollection(receiver.BclValueKind))
        {
            return FoldOutcome.Error(
                OperandTypeCode,
                "Element access requires one string, array, or regex-collection receiver.");
        }
        ...
        if (!TryImplicitInt32(argument.Operand, out var index))
        {
            return FoldOutcome.Error(
                ...
                receiver.BclValueKind == BclValueKind.GroupCollection
                    ? "A group index must be an exact Int32 value or a group-name string."
                    : "A regex collection index must be an exact Int32 value.");
        }
