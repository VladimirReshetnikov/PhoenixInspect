...
public static partial class ExpressionEvaluator
{
    ...
    /// Version from two to four components or its dotted string form. Every other creation stays not-folded.
    /// </summary>
    ...
    /// <summary>Constructs one BCL value from folded arguments, with exact constructor semantics.</summary>
    ...
    private static FoldOutcome ComputeBclValueBinary(SyntaxKind kind, Operand left, Operand right)
    {
        ...
        // equality and the language's reference equality agree; the regex family defines no deterministic operator.
        ...
        if (!TryCompareBclValue(left, right, out var comparison))
        {
            return FoldOutcome.Error(
                ...
                + "Regex family defines no deterministic operators.");
        }
