...
/// <summary>
/// One caller-referenced assembly whose metadata participates in constant evaluation: literal fields, enum
...
/// </remarks>
public sealed class ConstantReferenceAssembly
{
    private readonly ImmutableArray<byte> metadataBytes;

    private ConstantReferenceAssembly(
        string displayName,
        string? alias,
    ...
    public static ConstantReferenceAssembly? TryLoad(string path, string? alias, out string? error)
    {
        ...
        try
        {
            ...
            error = null;
            return new ConstantReferenceAssembly(
