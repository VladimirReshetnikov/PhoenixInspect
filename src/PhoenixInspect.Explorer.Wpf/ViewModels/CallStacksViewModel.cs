...
/// <remarks>
/// The adapter selects one frame at a time by snapshot-scoped ordinal, so this panel probes ordinals rather than
...
public sealed class CallStacksViewModel : ObservableObject
{
    ...
    /// <summary>Creates the call-stack panel.</summary>
    ...
    /// <summary>Clears every probed thread so the panel matches a newly opened or closed dump.</summary>
    public void Reset()
    {
        Threads.Clear();
        loadedThreads.Clear();
