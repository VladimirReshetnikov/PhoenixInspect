...
using ExternalTypeAlias = requestlib::PhoenixInspect.W8AliasTarget.ExternalRequestContext;
using ForwardedTypeAlias = PhoenixInspect.W8AliasTarget.ForwardedRequestContext;
using IntMatrixSlot = PhoenixInspect.W8TestTarget.GenericSlot<int[,]>;
using ExternalRequestSlot = requestlib::PhoenixInspect.W8AliasTarget.ExternalSlot<requestlib::PhoenixInspect.W8AliasTarget.ExternalRequestContext>;
using LocalTypeAlias = PhoenixInspect.W8TestTarget.RequestContext;
using NamedRvaStorage = PhoenixInspect.W8NamedRvaTarget.NamedRvaStorage;
using RequestMatrixAlias = PhoenixInspect.W8TestTarget.RequestContext[,];
using RequestMatrixSlot = PhoenixInspect.W8TestTarget.GenericSlot<PhoenixInspect.W8TestTarget.RequestContext[,]>;
using RequestSlot = PhoenixInspect.W8TestTarget.GenericSlot<PhoenixInspect.W8TestTarget.RequestContext>;
using RequestVectorAlias = PhoenixInspect.W8TestTarget.RequestContext[];
using RequestVectorSlot = PhoenixInspect.W8TestTarget.GenericSlot<PhoenixInspect.W8TestTarget.RequestContext[]>;
using static PhoenixInspect.W8TestTarget.NonGenericImports;
using static PhoenixInspect.W8TestTarget.StaticImports<PhoenixInspect.W8TestTarget.RequestContext>;
...
public static class W8TruthGate
{
    ...
    public static int Run(string profile)
    {
        ...
        requestlib::PhoenixInspect.W8AliasTarget.IExternalInterfaceSlot<
            requestlib::PhoenixInspect.W8AliasTarget.ExternalRequestContext>.Sentinel = unchecked((int)0xDD0D7A01);
        requestlib::PhoenixInspect.W8AliasTarget.FriendVisibleOwner.Sentinel = unchecked((int)0xDD0D7A02);
        var nonFriendAccessibility = PhoenixInspect.W8ForwarderTarget.NonFriendAccessibilityOwner.ReadAllForFixture();
    ...
    private static void MaterializeCollectibleConstruction()
    {
        ...
        var sourceAssemblyPath = typeof(
            requestlib::PhoenixInspect.W8AliasTarget.ExternalRequestContext).Assembly.Location;
        var loadedAssembly = loadContext.LoadFromAssemblyPath(sourceAssemblyPath);
        var argumentType = loadedAssembly.GetType(
            "PhoenixInspect.W8AliasTarget.ExternalRequestContext",
            throwOnError: true,
            ignoreCase: false)!;
