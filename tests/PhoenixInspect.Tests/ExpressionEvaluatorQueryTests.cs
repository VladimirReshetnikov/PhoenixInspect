...
/// and rendering.
/// </summary>
public sealed class ExpressionEvaluatorQueryTests
{
    ...
    public void Query_expressions_translate_and_fold_exactly(string expression, string expectedText)
    {
        ...
        {
            ExpressionValueKind.Sequence => result.ValueText!,
    ...
    [Theory]
    [InlineData("new { a = 1, b = \"x\" }.a", ExpressionValueKind.Int32, "1")]
    [InlineData("new { a = 1, b = \"x\" }.b", ExpressionValueKind.String, "x")]
    [InlineData("new { DayOfWeek.Monday }.Monday", ExpressionValueKind.EnumMember, "Monday")]
    [InlineData("new { a = 1 }.Equals(new { a = 1 })", ExpressionValueKind.Boolean, "True")]
    [InlineData("new { a = 1 }.Equals(new { a = 2 })", ExpressionValueKind.Boolean, "False")]
    [InlineData("new { a = 1 }.Equals(new { b = 1 })", ExpressionValueKind.Boolean, "False")]
    [InlineData("new { a = 1 }.Equals(new { a = 1, b = 2 })", ExpressionValueKind.Boolean, "False")]
    [InlineData("new { a = 1, b = \"x\" }.ToString()", ExpressionValueKind.String, "{ a = 1, b = x }")]
    [InlineData("$\"{new { a = 1 }}\"", ExpressionValueKind.String, "{ a = 1 }")]
    [InlineData("new { Total = new[] { 1, 2 }.Sum(), Pair = (1, 2) }.Pair.Item2", ExpressionValueKind.Int32, "2")]
        ...
        string expected)
    {
        ...
        var actual = expectedKind switch
        {
            ...
            ExpressionValueKind.Int32 => result.Int32Value!.Value.ToString(
                System.Globalization.CultureInfo.InvariantCulture),
            ExpressionValueKind.EnumMember => result.EnumMemberName!,
    ...
    {
        var result = ExpressionEvaluator.Evaluate(session: null, "new { Name = \"a\", Total = 3 }");
        Assert.Equal(ExpressionValueKind.Anonymous, result.Kind);
        ...
        var sequence = ExpressionEvaluator.Evaluate(
            session: null,
            "from x in new[] { 1, 2 } select new { x, Square = x * x }");
        Assert.Equal(ExpressionValueKind.Sequence, sequence.Kind);
    ...
    {
        var groups = ExpressionEvaluator.Evaluate(
            session: null,
            "new[] { 1, 2, 3, 4, 5 }.GroupBy(x => x % 2)");
        Assert.Equal(ExpressionValueKind.Sequence, groups.Kind);
        ...
        var first = ExpressionEvaluator.Evaluate(
            session: null,
    ...
    [Theory]
    [InlineData("new { a = 1 } == new { a = 1 }", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("new { 1 * 2 }", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("new { a = 1, a = 2 }", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("new[] { 1, 2 }.ThenBy(x => x)", "EVAL_LAMBDA_UNSUPPORTED")]
