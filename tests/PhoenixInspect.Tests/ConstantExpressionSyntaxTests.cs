...
/// </summary>
public sealed class ConstantExpressionSyntaxTests
{
    ...
    [InlineData("unchecked(int.MaxValue + 1)", "CONSTANT_UNCHECKED_UNSUPPORTED")]
    [InlineData("checked(int.MaxValue + 1)", "System.OverflowException")]
    [InlineData("1 switch { var x => 2 }", "CONSTANT_PATTERN_UNSUPPORTED")]
    [InlineData("\"a\" switch { string s => 1, _ => 0 }", "CONSTANT_PATTERN_UNSUPPORTED")]
    [InlineData("\"a\" is > 3", "CONSTANT_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("5 is \"a\"", "CONSTANT_OPERAND_TYPE_UNSUPPORTED")]
