...
/// </summary>
public sealed class ExpressionEvaluatorTupleTests
{
    ...
    public void Tuple_literals_fold_exactly(string expression, string expectedTypeName, string expectedText)
    {
        ...
        Assert.Equal(ExpressionValueKind.Tuple, result.Kind);
    ...
    [Theory]
    [InlineData("(1, \"a\").Item1", ExpressionValueKind.Int32, "1")]
    [InlineData("(1, \"a\").Item2", ExpressionValueKind.String, "a")]
    [InlineData("(count: 5, name: \"x\").count", ExpressionValueKind.Int32, "5")]
    [InlineData("(count: 5, name: \"x\").Item2", ExpressionValueKind.String, "x")]
    [InlineData("((1, 2), 3).Item1.Item2", ExpressionValueKind.Int32, "2")]
    [InlineData("(1, 2) == (1, 2)", ExpressionValueKind.Boolean, "True")]
    [InlineData("(1, 2) == (1, 3)", ExpressionValueKind.Boolean, "False")]
    [InlineData("(1, 2) != (1, 3)", ExpressionValueKind.Boolean, "True")]
    [InlineData("(1, 2L) == (1, 2)", ExpressionValueKind.Boolean, "True")]
    [InlineData("((1, 2), \"a\") == ((1, 2), \"a\")", ExpressionValueKind.Boolean, "True")]
    [InlineData("(1, \"a\").Equals((1, \"a\"))", ExpressionValueKind.Boolean, "True")]
    [InlineData("(1, \"a\").ToString()", ExpressionValueKind.String, "(1, a)")]
    [InlineData("((1, 2), 3).ToString()", ExpressionValueKind.String, "((1, 2), 3)")]
    [InlineData("$\"pair {(1, 2)}\"", ExpressionValueKind.String, "pair (1, 2)")]
    [InlineData("(1, 2).Item1 + (3, 4).Item2", ExpressionValueKind.Int32, "5")]
    ...
    public void Tuple_stops_are_typed(string expression)
    {
        ...
        Assert.Equal("EVAL_OPERAND_TYPE_UNSUPPORTED", result.DiagnosticCode);
    }
    ...
    {
        var tuple = ExpressionEvaluator.Evaluate(session: null, "(1, \"a\", (2, 3))");
        ...
        var named = ExpressionEvaluator.Evaluate(session: null, "(count: 7, name: \"x\")");
        Assert.Equal(["count", "name"], named.Children.Select(static child => child.Name).ToArray());

        var sequence = ExpressionEvaluator.Evaluate(session: null, "new[] { 10, 20, 30 }");
        ...
        var scalar = ExpressionEvaluator.Evaluate(session: null, "1 + 2");
        ...
        var bounded = ExpressionEvaluator.Evaluate(session: null, "Enumerable.Range(0, 600)");
