...
public sealed class ExpressionEvaluatorTests
{
    ...
    public void Char_results_evaluate_exactly(string expression, char expected)
    {
        var result = ExpressionEvaluator.Evaluate(session: null, expression);
        Assert.Equal(ExpressionEvaluationStatus.Exact, result.Status);
        Assert.Equal(ExpressionValueKind.Char, result.Kind);
    ...
    [InlineData("\"abc\"[^(-1)..]", "System.ArgumentOutOfRangeException")]
    [InlineData("1[0..1]", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("\"abc\"[\"x\"..]", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    ...
    [InlineData("Math.ReciprocalEstimate(2.0)", "EVAL_MEMBER_UNSUPPORTED")]
    [InlineData("Math.Sqrt(2m)", "EVAL_MEMBER_UNSUPPORTED")]
    [InlineData("1m + 0.5", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("1.5 & 2", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("-1 + 1UL", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("-(1UL)", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("(bool)1", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("(int)\"5\"", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("(int)null", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("\"abc\".ToLower()", "EVAL_CULTURE_SENSITIVE_UNSUPPORTED")]
    [InlineData("\"abc\".ToUpper()", "EVAL_CULTURE_SENSITIVE_UNSUPPORTED")]
    [InlineData("\"abc\".IndexOf(\"b\")", "EVAL_CULTURE_SENSITIVE_UNSUPPORTED")]
    [InlineData("\"abc\".StartsWith(\"a\")", "EVAL_CULTURE_SENSITIVE_UNSUPPORTED")]
    [InlineData("char.ToUpper('a')", "EVAL_CULTURE_SENSITIVE_UNSUPPORTED")]
    [InlineData("\"abc\".CompareTo(\"abd\")", "EVAL_CULTURE_SENSITIVE_UNSUPPORTED")]
    [InlineData("\"abc\".GetHashCode()", "EVAL_MEMBER_UNSUPPORTED")]
    [InlineData("\"abc\".NoSuchMethod()", "EVAL_MEMBER_UNSUPPORTED")]
    [InlineData("string.Format(\"{0}\", \"x\")", "EVAL_MEMBER_UNSUPPORTED")]
    [InlineData("true + 1", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("\"a\" < \"b\"", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("!1", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("1 && true", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("1 ? 1 : 2", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("1 ? 2 : 3", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    public void Errors_are_typed_stops(string expression, string expectedCode)
    {
        ...
        Assert.Equal(ExpressionValueKind.None, result.Kind);
    ...
    public void Sequence_scalars_evaluate_exactly(string expression, string expected)
    {
        ...
        var rendered = result.Kind switch
        {
            ...
            ExpressionValueKind.String => "\"" + result.StringValue + "\"",
            _ => result.ValueText,
        };
    ...
    [InlineData("Enumerable.Range(0, 5000)", "EVAL_SEQUENCE_BOUND_EXCEEDED")]
    [InlineData("Enumerable.Repeat(1, -1)", "System.ArgumentOutOfRangeException")]
    [InlineData("new[] { \"b\", \"a\" }.Order()", "EVAL_CULTURE_SENSITIVE_UNSUPPORTED")]
    [InlineData("new[] { \"b\", \"a\" }.Max()", "EVAL_CULTURE_SENSITIVE_UNSUPPORTED")]
    [InlineData("new[] { \"a\", 'b' }", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("new[] { \"a\" }.Sum()", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
