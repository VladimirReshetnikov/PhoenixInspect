...
/// </summary>
public sealed class ConstantExpressionLambdaTests
{
    ...
    [Theory]
    [InlineData("new[] { 1 }.Select(x => { return x; })", "CONSTANT_LAMBDA_UNSUPPORTED")]
    [InlineData("new[] { 1 }.Select(static x => x)", "CONSTANT_LAMBDA_UNSUPPORTED")]
    [InlineData("new[] { 1 }.Sum((x, i) => x + i)", "CONSTANT_LAMBDA_UNSUPPORTED")]
    [InlineData("new[] { 1 }.Aggregate((a, b) => a + b)", "CONSTANT_LAMBDA_UNSUPPORTED")]
    [InlineData("\"abc\".Select(c => c)", "CONSTANT_LAMBDA_UNSUPPORTED")]
    [InlineData("new[] { 1, 2 }.Where(x => x + 1)", "CONSTANT_OPERAND_TYPE_UNSUPPORTED")]
    ...
    [InlineData("new[] { \"b\", \"a\" }.OrderBy(s => s)", "CONSTANT_CULTURE_SENSITIVE_UNSUPPORTED")]
    ...
    public void Lambda_parameters_scope_lexically()
    {
        ...
        Assert.Equal(ConstantExpressionStatus.Exact, nested.Status);
        Assert.Equal("{ 30, 60 }", nested.ValueText);

        var shadowed = ConstantExpressionEvaluator.Evaluate(
            session: null,
            "new[] { 1, 2 }.Select(x => new[] { 5 }.Select(x => x).Sum() + x).ToArray()");
        Assert.Equal(ConstantExpressionStatus.Exact, shadowed.Status);
        Assert.Equal("{ 6, 7 }", shadowed.ValueText);

        var unbound = ConstantExpressionEvaluator.Evaluate(session: null, "new[] { 1 }.Select(x => y)");
        Assert.Equal(ConstantExpressionStatus.NotConstant, unbound.Status);

        var outsideScope = ConstantExpressionEvaluator.Evaluate(session: null, "new[] { 1 }.Select(x => x).Sum() + x");
        Assert.Equal(ConstantExpressionStatus.NotConstant, outsideScope.Status);
    }
}
