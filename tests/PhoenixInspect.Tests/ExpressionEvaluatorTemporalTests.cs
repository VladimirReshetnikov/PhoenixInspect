...
/// </summary>
public sealed class ExpressionEvaluatorTemporalTests
{
    ...
    public void Temporal_values_fold_exactly(string expression, string expectedTypeName, string expectedText)
    {
        ...
        Assert.Equal(ExpressionValueKind.Temporal, result.Kind);
    ...
    {
        var formatted = ExpressionEvaluator.Evaluate(
            session: null, "new DateTime(2026, 7, 30).ToString(\"yyyy-MM-dd\")");
        Assert.Equal(ExpressionEvaluationStatus.Exact, formatted.Status);
        Assert.Equal("2026-07-30", formatted.StringValue);

        var interpolated = ExpressionEvaluator.Evaluate(
            session: null, "$\"stalled for {TimeSpan.FromMilliseconds(30045):c}\"");
        Assert.Equal(ExpressionEvaluationStatus.Exact, interpolated.Status);
        Assert.Equal("stalled for 00:00:30.0450000", interpolated.StringValue);

        var dayOfWeek = ExpressionEvaluator.Evaluate(
            session: null, "new DateTime(2026, 7, 30).AddDays(2).DayOfWeek");
        Assert.Equal(ExpressionEvaluationStatus.Exact, dayOfWeek.Status);
        Assert.Equal(ExpressionValueKind.EnumMember, dayOfWeek.Kind);
    ...
    {
        var projected = ExpressionEvaluator.Evaluate(
            session: null, "new[] { 1200, 30045 }.Select(ms => TimeSpan.FromMilliseconds(ms))");
        Assert.Equal(ExpressionEvaluationStatus.Exact, projected.Status);
        Assert.Equal("TimeSpan[]", projected.ValueTypeName);
        Assert.Equal("{ 00:00:01.2000000, 00:00:30.0450000 }", projected.ValueText);

        var maximum = ExpressionEvaluator.Evaluate(
            session: null, "new[] { 3, 1, 2 }.Select(d => new DateTime(2026, 7, d)).Max()");
        Assert.Equal(ExpressionEvaluationStatus.Exact, maximum.Status);
        Assert.Equal(ExpressionValueKind.Temporal, maximum.Kind);
        Assert.Equal("2026-07-03T00:00:00.0000000", maximum.ValueText);

        var ordered = ExpressionEvaluator.Evaluate(
            session: null, "new[] { 2, 1 }.OrderBy(d => new DateTime(2026, 7, d))");
        Assert.Equal(ExpressionEvaluationStatus.Exact, ordered.Status);
        Assert.Equal("{ 1, 2 }", ordered.ValueText);

        var thresholds = ExpressionEvaluator.Evaluate(
            session: null,
            "new[] { 1200, 30045 }.Count(ms => TimeSpan.FromMilliseconds(ms) > TimeSpan.FromSeconds(2))");
        Assert.Equal(ExpressionEvaluationStatus.Exact, thresholds.Status);
    ...
    [Theory]
    [InlineData("DateTime.Now", "EVAL_NONDETERMINISTIC_UNSUPPORTED")]
    [InlineData("DateTime.UtcNow", "EVAL_NONDETERMINISTIC_UNSUPPORTED")]
    [InlineData("DateTime.Today", "EVAL_NONDETERMINISTIC_UNSUPPORTED")]
    [InlineData("DateTimeOffset.Now", "EVAL_NONDETERMINISTIC_UNSUPPORTED")]
    [InlineData("new DateTime(2026, 7, 30).ToLocalTime()", "EVAL_NONDETERMINISTIC_UNSUPPORTED")]
    [InlineData("new DateTime(2026, 7, 30).ToUniversalTime()", "EVAL_NONDETERMINISTIC_UNSUPPORTED")]
    [InlineData("DateTime.Parse(\"2026-07-30\")", "EVAL_CULTURE_SENSITIVE_UNSUPPORTED")]
    ...
    [InlineData("new TimeSpan(1, 0, 0) + 5", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("new DateTime(2026, 7, 30) + new DateTime(2026, 7, 30)", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
    [InlineData("new DateTime(2026, 7, 30) == new DateOnly(2026, 7, 30)", "EVAL_OPERAND_TYPE_UNSUPPORTED")]
