...
/// </summary>
public sealed class ExpressionEvaluatorReflectionTests
{
    ...
    [Theory]
    [InlineData("typeof(Math).GetMethod(\"Sqrt\").Name", ExpressionValueKind.String, "Sqrt")]
    [InlineData("typeof(Math).GetMethod(\"Sqrt\").IsStatic", ExpressionValueKind.Boolean, "True")]
    [InlineData("typeof(Math).GetMethod(\"Sqrt\").IsPublic", ExpressionValueKind.Boolean, "True")]
    [InlineData(
    ...
    [InlineData(
        ...
        ExpressionValueKind.Int32,
        "0")]
    [InlineData("typeof(string).GetProperty(\"Length\").Name", ExpressionValueKind.String, "Length")]
    [InlineData("typeof(string).GetProperty(\"Length\").CanRead", ExpressionValueKind.Boolean, "True")]
    [InlineData("typeof(string).GetProperty(\"Length\").CanWrite", ExpressionValueKind.Boolean, "False")]
    [InlineData("typeof(int).GetField(\"MaxValue\").IsLiteral", ExpressionValueKind.Boolean, "True")]
    [InlineData("typeof(int).GetField(\"MaxValue\").IsStatic", ExpressionValueKind.Boolean, "True")]
    [InlineData(
    ...
    [InlineData(
        ...
        ExpressionValueKind.Int32,
        "1")]
    [InlineData("typeof(string).GetMethod(\"NoSuchMethod\") == null", ExpressionValueKind.Boolean, "True")]
    [InlineData(
        "typeof(DateTime).GetProperties().Select(p => p.Name).Contains(\"Year\")",
        ExpressionValueKind.Boolean,
        "True")]
    [InlineData("typeof(Version).GetConstructors().Length > 1", ExpressionValueKind.Boolean, "True")]
    [InlineData("typeof(TimeSpan).GetFields().Any(f => f.Name == \"Zero\")", ExpressionValueKind.Boolean, "True")]
    [InlineData(
        "typeof(Math).GetMember(\"PI\").Length",
        ExpressionValueKind.Int32,
        "1")]
    public void Reflection_info_folds_exactly(string expression, ExpressionValueKind expectedKind, string expected)
    {
    ...
    public void Member_lists_are_canonically_ordered()
    {
        ...
        Assert.Equal(ExpressionEvaluationStatus.Exact, first.Status);
